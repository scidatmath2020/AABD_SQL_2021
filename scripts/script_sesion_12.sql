...
SELECT AGE('2014-04-01','2014-01-01 14:00'), AGE(TIMESTAMP '2014-04-25');

SELECT AGE('2021-12-16 13:37:00','2014-04-25 17:00:10');
...
SELECT order_line, ship_date, order_date,EXTRACT(epoch FROM AGE(ship_date, order_date))/86400 as tiempo
FROM sales ORDER BY tiempo DESC
...
SELECT * FROM customer WHERE customer_name LIKE 'A% ____';
...
SELECT sales, TO_CHAR(sales,'0999.99')
FROM sales;


SELECT sales, TO_CHAR(sales,'SG$SG9 999.99')
FROM sales;

SELECT order_date, TO_CHAR(order_date,'MMDDYY')
FROM sales;

--December Friday 17, 2021

SELECT order_date, TO_CHAR(order_date,'Month Day DD, YYYY')
FROM sales;
...
SELECT usename FROM pg_user; -- username; usename
...
create table mi_tabla as
	select order_id, order_date, extract() from sales
...
select * from customer where customer_id = 'HP-14815';

select * from customer where city = 'Los Angeles';

select * from sales where ship_mode = 'Second Class' order by order_date;

truncate customer;
create table customer_20_60 as select * from customer where age between in 20 and 60;

truncate customer; -- delete * from customer;
drop table customer;
...
select * from cuentas;
call transferir(2,8,2000), transferir (1,3,200);
